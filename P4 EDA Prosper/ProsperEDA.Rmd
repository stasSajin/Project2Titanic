...
p_no_duplicates$ListCreationYear<-as.factor(year
                                (p_no_duplicates$ListingCreationDate))
...
year_returns<-p_no_duplicates %>%
     ...
     summarise(meanReturn=mean(EstimatedReturn),
            ...
            n=n(),
            LCI=(mean(EstimatedReturn)-(1.96*sd(EstimatedReturn)/
                                            length(EstimatedReturn))),
            UCI=(mean(EstimatedReturn)+(1.96*sd(EstimatedReturn)/
                                            length(EstimatedReturn))))
```
...
ggplot(Returns, aes(ListCreationYear, EstimatedReturn)) + 
    ...
    theme(legend.position = "none")+
    scale_y_continuous(breaks=seq(-.2, .3, .05))+
    geom_point(color=ifelse(Returns$EstimatedReturn<=
                                quantile(Returns$EstimatedReturn,.05),
                    ifelse(Returns$EstimatedReturn<=
                               quantile(Returns$EstimatedReturn, .01),
                           "red", "orange"),"black"))+
...
ggplot(pastLoans, aes(ListCreationYear, AnnualizedReturn)) + 
    geom_boxplot(aes(fill = ListCreationYear))+
    theme(legend.position = "none")+
    ggtitle("Actual Returns from Past Loans 
            (Includes Completed, Charged Off, and Defaults)")+
    stat_summary(fun.y=mean, color="black", geom="point", 
               shape=18, size=3,show_guide = FALSE)+
    stat_summary(fun.y=mean, colour="black", geom="text", show_guide = FALSE, 
...
loanTypeStats<-pastLoans %>% filter(!is.na(AnnualizedReturn)) %>%
    ...
    n=n(),
    LCI=(mean(AnnualizedReturn)-(1.96*sd(AnnualizedReturn)/
                                     length(AnnualizedReturn))),
    UCI=(mean(AnnualizedReturn)+(1.96*sd(AnnualizedReturn)/
                                     length(AnnualizedReturn))))

a<-pastLoans %>% select(LP_CustomerPayments,LP_CustomerPrincipalPayments,
                        LP_InterestandFees) %>%
    mutate(total=(LP_CustomerPrincipalPayments+LP_InterestandFees)-
               LP_CustomerPayments)
...
pastLoans$`ProsperRating (numeric)`<-as.factor(
...
cor.test(pastLoans$AnnualizedReturn,
         as.integer(pastLoans$ProsperScore),method="spearman",
         conf.level = 0.95)
```
...
```{r}
p1<-ggplot(subset(pastLoans,!is.na(EstimatedReturn)), 
           aes(x=`ProsperRating (numeric)`, y=AnnualizedReturn))+ 
    geom_boxplot()+
    theme(legend.position = "none")+
    scale_y_continuous(breaks=seq(-1, .3, .05))+
    ggtitle("Annualized Returns for Each Prosper Score")
...
treasuryRates<-Quandl("USTREASURY/YIELD", trim_start="2005-01-01", 
                      trim_end="2015-01-01")
...
###combine the dataframes pastLoans and treasuryRatesDaily based on 
#dayOriginationColumn
merged<-left_join(pastLoans, treasuryRatesDaily, by="dayOrigination")

#calculate the sharpe ratio for each loan; note that I divided the treasury 
#yield by 100 to convert it into proportion.
...
ggplot(data=noMissing, aes(x=sqrt(DebtToIncomeRatio)))+
    ...
    scale_x_continuous(limits = 
                           c(0, quantile(noMissing$DebtToIncomeRatio,.99)))+
...
merged$date_diff <- difftime(merged$ClosedDate ,merged$LoanOriginationDate , 
                             units = c("days"))/30
